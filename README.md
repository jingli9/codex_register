...
直接在codex上进行注册会被要求添加手机号码，据说如果ip够干净能跳过，但是干净的ip是用一个少一个。所以要是最终方法是找一个接码平台，一个号码就几毛钱。如果有需要，我会更新对接接码的（我是不会用的，白嫖进行到底）

不添加手机号的方式也研究出来了（不是PoW），但是不太想很快公开，先等等看吧。下图为不需要手机号不用PoW的截图
